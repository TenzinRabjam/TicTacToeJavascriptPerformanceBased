var firstName = prompt("Enter Player one's name.");
var secondName = prompt("Enter Player two's name.");
var turn = 'X';
var score = {
   'X': 0,
   'O': 0
...
function fnChoose(e) {
    if (e.target && e.target.nodeName == "TD") {
        ...
        if ((targetElement.className).indexOf("disabled") == -1) {
			...
			if(prevTurn == "X")
				document.getElementById("output").innerHTML = firstName  + "'s turn"; 
            else{
				document.getElementById("output").innerHTML = secondName  + "'s turn"; 
			}	
			...
			if (fndecide(targetElement, prevTurn)) {
				...
				if(prevTurn == "X"){
					...
					document.getElementById("output").innerHTML = firstName + " has won!"; 
				...
				if(prevTurn == "O"){
					...
					document.getElementById("output").innerHTML = secondName + " has won!"; 
